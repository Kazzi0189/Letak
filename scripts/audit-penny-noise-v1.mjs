...
function detectNoise(offer) {
  ...
  for (const rule of NOISE_PATTERNS) {
    ...
    if (rule.pattern.test(value)) {
      // Missing category / priceText je jen slabý signál; ne vždy chyba.
...
async function main() {
  ...
  await writeFile(`${OUTPUT_DIR}/penny-noise-audit-v1-summary.json`, JSON.stringify(report, null, 2) + "\n", "utf8");

  console.log("Penny noise audit v1 finished.");
