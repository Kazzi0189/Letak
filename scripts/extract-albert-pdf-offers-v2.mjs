...
async function downloadFile(url, outputPath) {
  const response = await fetch(url, {
    redirect: "follow",
    headers: {
      "user-agent": "Mozilla/5.0 (compatible; LetakovyPorovnavacAlbertPdfOffersV2/0.1; +https://github.com/)",
...
function normalizeComputedPrice(price) {
  ...
  // Jednotkové ceny v letáku jsou zaokrouhlené, takže dopočet často dá 39,91 místo reálných 39,90.
...
function extractPriceTexts(text) {
  return unique(
    normalizeText(text).match(/\b\d{1,4}(?:\s?\d{3})?[,.]\d{2}\s*Kč|\b\d{1,4},-|\b\d{1,4}[,.]\d{2}\b/giu) || []
  );
}
...
function isMarketingLine(line) {
  return /^(NEPORAZITELNÉ|BĚŽNÁ CENA|BEZ|APLIKACE|NOVINKA|VÍCE AKCÍ|EXTRA LETÁK|PLATÍ|POUZE|www\.albert\.cz|Od \d|A$|▼|Tato nabídka|Nelze garantovat|produktů označených|NEPORA ZITELNÉ produkty|S APLIKACÍ|KUPÓNY|KREDITY|JEŠTĚ NEMÁTE)/iu.test(line);
}
...
function cleanProductName(lines) {
  ...
  for (const prefix of KNOWN_NOISE_PREFIXES) {
    text = text.replace(new RegExp(`^${prefix.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s+`, "iu"), "");
  }

  // Typické spojení dvou sousedních produktů z více sloupců: u názvu ponecháme pravou, konkrétnější část.
  const productSplitHints = [
    /^(?:Nivea Antiperspirant sprej\s+)?Božkov Originál$/iu,
    /^(?:Kuře bez drobů Albert Listové těsto rozválené\s+)?Gambrinus Original 10$/iu,
  ];

  for (const hint of productSplitHints) {
...
function looksLikeProductName(product) {
  ...
  if (/^(od|do|kg|g|ml|l|ks|role|cena|za|bez|aplikace|vybrané druhy|různé druhy|nápoj|voda)$/iu.test(product)) return false;
...
function extractDescription(bulletLines) {
  const parts = bulletLines
    ...
    .filter((line) => !/^(vybrané druhy|různé druhy|bez Aplikace|od \d|záloha|cena za|limitovaná edice)/iu.test(line));
...
function explicitMainPriceFromBullet(bulletText, unitInfo) {
  ...
  // Poslední cena za odrážkou je v Albert PDF obvykle hlavní cena produktu.
...
function classifyQuality(product, price, unitInfo, packageSize, rawContext) {
  const reasons = [];
  ...
  if (price != null && price < 2 && !/kg|l|100 g|100 ml|cena za/iu.test(rawContext)) reasons.push("podezřele nízká cena");
  if (/(\b\w+\b.*){9,}/u.test(product) && product.split(/\s+/).length > 8) reasons.push("moc dlouhý/slitý název");
  if (/•.*•.*•.*•.*•.*•/u.test(rawContext) && product.split(/\s+/).length > 4) reasons.push("možné sloučení více produktů");
...
function parsePage(pageText, pageNumber, pdf) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const rawContext = [...nameLines, ...bulletLines].join(" ").slice(0, 500);
    const quality = classifyQuality(product, price, unitInfo, packageSize, rawContext);
    ...
    summary: {
      ...
      recommendedPath:
        deduped.filter((offer) => !offer.suspect).length > 120
          ? "build-import-albert-from-pdf-v2"
          : deduped.length > 60
            ? "inspect-pdf-offers-v2"
...
async function main() {
  ...
  const output = {
    meta: {
      ...
      cleanCount: combined.filter((offer) => !offer.suspect).length,
      suspectCount: combined.filter((offer) => offer.suspect).length,
      parser: "scripts/extract-albert-pdf-offers-v2.mjs",
      note: "Průzkumný parser z PDF textu. V2 zpřísňuje suspect pravidla a normalizuje ceny dopočtené z jednotkové ceny.",
    },
    offers: combined,
  };
  ...
  const summary = {
    ...
    summary: {
      ...
      recommendedPath:
        combined.filter((offer) => !offer.suspect).length > 180
          ? "turn-v2-into-import-albert"
          : combined.length > 80
            ? "inspect-and-tighten-v2"
            : "needs-parser-improvement",
    ...
    suspectSampleOffers: combined.filter((offer) => offer.suspect).slice(0, 120),
  };

  await writeFile(IMPORT_OUTPUT, JSON.stringify(output, null, 2) + "\n", "utf8");
  await writeFile(`${OUTPUT_DIR}/albert-pdf-offers-v2-debug.json`, JSON.stringify({ summary, results }, null, 2) + "\n", "utf8");
  await writeFile(`${OUTPUT_DIR}/pdf-offers-v2-summary.json`, JSON.stringify(summary, null, 2) + "\n", "utf8");

  console.log("Albert PDF offers extraction v2 finished.");
  console.log(JSON.stringify(summary.summary, null, 2));
  console.log(`Wrote ${OUTPUT_DIR}/pdf-offers-v2-summary.json`);
}
