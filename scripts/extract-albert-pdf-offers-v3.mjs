...
function isMarketingLine(line) {
  return /^(NEPORAZITELNÉ|BĚŽNÁ CENA|BEZ|APLIKACE|NOVINKA|VÍCE AKCÍ|EXTRA LETÁK|PLATÍ|POUZE|www\.albert\.cz|Od \d|A$|▼|Tato nabídka|Nelze garantovat|produktů označených|NEPORA ZITELNÉ produkty|S APLIKACÍ|KUPÓNY|KREDITY|JEŠTĚ NEMÁTE|NABÍDKA PRO NEJMENŠÍ|VE ZNAČKOVÉ KVALITĚ)/iu.test(line);
}
    ...
    summary: {
      ...
      recommendedPath:
        deduped.filter((offer) => !offer.suspect).length > 120
          ? "build-import-albert-from-pdf-v3-clean-only"
          : deduped.length > 60
...
async function main() {
  ...
  const output = {
    meta: {
      ...
      parser: "scripts/extract-albert-pdf-offers-v3.mjs",
      note: "Průzkumný parser z PDF textu. V3 zpřísňuje suspect pravidla a vytváří i clean-only výstup pro budoucí import.",
    },
    offers: combined,
  };
    ...
    summary: {
      ...
      recommendedPath:
        cleanOnly.length > 180
          ? "turn-v3-clean-only-into-import-albert"
          : cleanOnly.length > 100
            ? "inspect-v3-clean-and-then-import-clean-only"
            : "needs-parser-improvement",
  ...
  await writeFile(`${OUTPUT_DIR}/albert-pdf-offers-v3-debug.json`, JSON.stringify({ summary, results }, null, 2) + "\n", "utf8");
  await writeFile(`${OUTPUT_DIR}/pdf-offers-v3-summary.json`, JSON.stringify(summary, null, 2) + "\n", "utf8");

  console.log("Albert PDF offers extraction v3 finished.");
  console.log(JSON.stringify(summary.summary, null, 2));
  console.log(`Wrote ${OUTPUT_DIR}/pdf-offers-v3-summary.json`);
}
