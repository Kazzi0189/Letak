    ...
    summary: {
      ...
      recommendedPath:
        deduped.filter((offer) => !offer.suspect).length > 120
          ? "build-import-albert-from-pdf-v4-clean-only"
          : deduped.length > 60
...
async function main() {
  ...
  const output = {
    meta: {
      ...
      parser: "scripts/extract-albert-pdf-offers-v4.mjs",
      note: "Průzkumný parser z PDF textu. V4 dál zpřísňuje clean-only výstup, odstraňuje další marketingové prefixy a přesouvá zjevně slité bloky do suspect.",
    },
    offers: combined,
  };
    ...
    summary: {
      ...
      recommendedPath:
        cleanOnly.length > 180
          ? "turn-v4-clean-only-into-import-albert"
          : cleanOnly.length > 100
            ? "inspect-v4-clean-and-then-import-clean-only"
            : "needs-parser-improvement",
  ...
  await writeFile(`${OUTPUT_DIR}/albert-pdf-offers-v4-debug.json`, JSON.stringify({ summary, results }, null, 2) + "\n", "utf8");
  await writeFile(`${OUTPUT_DIR}/pdf-offers-v4-summary.json`, JSON.stringify(summary, null, 2) + "\n", "utf8");

  console.log("Albert PDF offers extraction v4 finished.");
  console.log(JSON.stringify(summary.summary, null, 2));
  console.log(`Wrote ${OUTPUT_DIR}/pdf-offers-v4-summary.json`);
}
