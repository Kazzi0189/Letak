...
function hasConflictingExplicitPrice(price, rawContext, unitInfo) {
  ...
  const looksComputed = [20, 21, 22, 23, 24, 25].includes(decimal) || [91, 92, 93, 94].includes(decimal);

  return !hasClose && hasDifferentLikelyMainPrice && looksComputed;
}
    ...
    summary: {
      ...
      recommendedPath:
        deduped.filter((offer) => !offer.suspect).length > 120
          ? "build-import-albert-from-pdf-v6-clean-only"
          : deduped.length > 60
...
async function main() {
  ...
  const output = {
    meta: {
      ...
      parser: "scripts/extract-albert-pdf-offers-v6.mjs",
      note: "Průzkumný parser z PDF textu. V6 je finální čistící průchod před importem: vyřazuje konfliktní ceny, reklamní názvy a nesoulad balení v názvu a packageSize.",
    },
    offers: combined,
  };
    ...
    summary: {
      ...
      recommendedPath:
        cleanOnly.length > 180
          ? "turn-v6-clean-only-into-import-albert"
          : cleanOnly.length > 100
            ? "inspect-v6-clean-and-then-import-clean-only"
            : "needs-parser-improvement",
  ...
  await writeFile(`${OUTPUT_DIR}/albert-pdf-offers-v6-debug.json`, JSON.stringify({ summary, results }, null, 2) + "\n", "utf8");
  await writeFile(`${OUTPUT_DIR}/pdf-offers-v6-summary.json`, JSON.stringify(summary, null, 2) + "\n", "utf8");

  console.log("Albert PDF offers extraction v6 finished.");
  console.log(JSON.stringify(summary.summary, null, 2));
  console.log(`Wrote ${OUTPUT_DIR}/pdf-offers-v6-summary.json`);
}
