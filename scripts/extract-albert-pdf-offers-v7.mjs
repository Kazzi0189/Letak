...
function explicitMainPricesFromContext(rawContext = "", unitInfo = null) {
  return extractPriceTexts(rawContext)
    .map((price) => toNumber(price))
    .filter((price) => price != null && price > 0 && price < 10000)
    .filter((price) => !unitInfo?.unitPrice || !numbersClose(price, unitInfo.unitPrice, 0.01));
}

function productContainsDifferentPackage(product = "", packageSize = "") {
  const productPackages = [...String(product).matchAll(/\b\d+(?:[,.]\d+)?\s*(?:ml|l|g|kg|ks|dávek)\b/giu)]
    .map((match) => match[0].replace(/\s+/g, " ").toLowerCase());

  if (!productPackages.length || !packageSize) return false;

  const normalizedPackage = String(packageSize).replace(/\s+/g, " ").toLowerCase();

  return productPackages.some((pkg) => !normalizedPackage.includes(pkg));
}

function hasConflictingExplicitPrice(price, rawContext, unitInfo) {
  const explicitPrices = explicitMainPricesFromContext(rawContext, unitInfo);
  if (!explicitPrices.length) return false;

  // Pokud se v textu vyskytuje skutečná hlavní cena, která není blízko price,
  // a zároveň price vypadá jako dopočet typu 199,20 / 249,20, raději to vyřadíme.
  const hasClose = explicitPrices.some((explicit) => numbersClose(explicit, price, 0.11));
  const hasDifferentLikelyMainPrice = explicitPrices.some((explicit) => {
    const diff = Math.abs(explicit - price);
    if (diff < 0.11) return false;
    if (explicit < 5) return false;
    return diff >= 0.5;
  });

  const decimal = Math.round((price - Math.floor(price)) * 100);
  const looksComputed = [0, 20, 21, 22, 23, 24, 25].includes(decimal) || [91, 92, 93, 94].includes(decimal);

  const nearestExplicitDiff = Math.min(...explicitPrices.map((explicit) => Math.abs(explicit - price)));
  const roundedConflict = nearestExplicitDiff >= 0.5 && nearestExplicitDiff <= 1.5 && Number.isInteger(price);

  return !hasClose && hasDifferentLikelyMainPrice && (looksComputed || roundedConflict);
}

function finalCleanRejectReasons(offer) {
  const reasons = [];
  const rawContext = offer.rawContext || "";
  const product = offer.product || "";

  if (hasHardMarketingNoise(product)) reasons.push("finální filtr: marketingový název");
  if (/^(VEZMĚTE SI|Z REGÁLU|S BODY|CENA BEZ BODŮ|PLATNÉM DO|HI T MĚSÍCE)/iu.test(product)) reasons.push("finální filtr: reklamní text místo produktu");
  if (productContainsDifferentPackage(product, offer.packageSize || "")) reasons.push("finální filtr: jiné balení v názvu než packageSize");
  ...
  if (hasConflictingExplicitPrice(offer.price, rawContext, unitInfo)) reasons.push("finální filtr: konflikt explicitní ceny a dopočtu");

  // Tvrdé známé zbytky z V5 ukázek.
  if (/^Šampon na vlasy\s+\d+\s*ml$/iu.test(product)) reasons.push("finální filtr: obecný název s konfliktním objemem");
  if (/^A MUŽEŠ JÍ\(S\)T!?$/iu.test(product)) reasons.push("finální filtr: reklamní slogan místo produktu");
  if (/^KREDIT NAVÍC\b/iu.test(rawContext) && product.split(/\s+/).length < 3) reasons.push("finální filtr: kreditový reklamní blok");
  if (/^TR\s+O\s+/iu.test(rawContext)) reasons.push("finální filtr: zbytkový OCR text v rawContext");
  if (/^PŘI KOUPI\b/iu.test(rawContext) && product.split(/\s+/).length < 3) reasons.push("finální filtr: promo text před produktem");
  if (/^Nápoje\b/iu.test(rawContext) && product.split(/\s+/).length < 2) reasons.push("finální filtr: kategoriový šum před produktem");
  if (/^\d{1,4}(?:[,.]\d{2})?\s*Kč$/iu.test(offer.description || "")) reasons.push("finální filtr: cena zůstala v description");
  if (/Dolce Gusto Kapsle/iu.test(product) && /199 Kč/iu.test(rawContext) && !numbersClose(offer.price, 199, 0.11)) reasons.push("finální filtr: Dolce Gusto cena konflikt");
  if (/Pampers Plenkové kalhotky/iu.test(product) && /279 Kč/iu.test(rawContext) && !numbersClose(offer.price, 279, 0.11)) reasons.push("finální filtr: Pampers cena konflikt");
  if (/Johnnie Walker Red Label/iu.test(product) && /329 Kč/iu.test(rawContext) && !numbersClose(offer.price, 329, 0.11)) reasons.push("finální filtr: Johnnie Walker cena konflikt");
  if (/Captain Morgan/iu.test(product) && /319 Kč/iu.test(rawContext) && !numbersClose(offer.price, 319, 0.11)) reasons.push("finální filtr: Captain Morgan cena konflikt");
  if (/Ben & Jerry/i.test(product) && /159 Kč/iu.test(rawContext) && !numbersClose(offer.price, 159, 0.11)) reasons.push("finální filtr: Ben & Jerry cena konflikt");
  if (/Jar Platinum/i.test(product) && /599 Kč/iu.test(rawContext) && !numbersClose(offer.price, 599, 0.11)) reasons.push("finální filtr: Jar cena konflikt");

  return unique(reasons);
}

function applyFinalCleanFilter(offers) {
  return offers.map((offer) => {
    const finalReasons = finalCleanRejectReasons(offer);

    if (!finalReasons.length) return offer;

    return {
      ...offer,
      confidence: "low",
      suspect: true,
      suspectReasons: unique([...(offer.suspectReasons || []), ...finalReasons]),
    };
  });
}
...
async function inspectPdf(pdf) {
  ...
  const deduped = applyFinalCleanFilter(dedupeOffers(allOffers));
    ...
    summary: {
      ...
      recommendedPath:
        deduped.filter((offer) => !offer.suspect).length > 120
          ? "build-import-albert-from-pdf-v7-clean-only"
          : deduped.length > 60
...
async function main() {
  ...
  const output = {
    meta: {
      ...
      parser: "scripts/extract-albert-pdf-offers-v7.mjs",
      note: "Průzkumný parser z PDF textu. V7 je poslední cílený čistící filtr před importem: odstraňuje reklamní názvy, ceny v popisu a konflikty explicitní ceny s dopočtem.",
    },
    offers: combined,
  };
    ...
    summary: {
      ...
      recommendedPath:
        cleanOnly.length > 180
          ? "turn-v7-clean-only-into-import-albert"
          : cleanOnly.length > 100
            ? "inspect-v7-clean-and-then-import-clean-only"
            : "needs-parser-improvement",
  ...
  await writeFile(`${OUTPUT_DIR}/albert-pdf-offers-v7-debug.json`, JSON.stringify({ summary, results }, null, 2) + "\n", "utf8");
  await writeFile(`${OUTPUT_DIR}/pdf-offers-v7-summary.json`, JSON.stringify(summary, null, 2) + "\n", "utf8");

  console.log("Albert PDF offers extraction v7 finished.");
  console.log(JSON.stringify(summary.summary, null, 2));
  console.log(`Wrote ${OUTPUT_DIR}/pdf-offers-v7-summary.json`);
}
