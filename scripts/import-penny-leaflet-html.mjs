...
function makeId(product, price, pageNumber) {
  return (
    "penny-leaflet-" +
    createHash("sha1")
      .update(`${product}|${price}|${pageNumber}`)
      .digest("hex")
      .slice(0, 16)
  );
}

function cleanText(text, pageNumber) {
  return text
    ...
    .replace(/nabídka Jedinečná/gi, " ")
    .replace(/ilustrační foto/gi, " ")
    .replace(/Nabídka platná od středy.*?2026/gi, " ")
...
function extractLeadPrices(text) {
  ...
  const prices = [];
  const regex = /(\d{1,4}(?:\s?\d{3})*,\d{2})\s*(?:Kč)?(?:\s+\d{1,4}(?:\s?\d{3})*,\d{2}\s*\/\s*\d+%)?/g;
  let match;
...
function isProbablyProductName(name) {
  if (!name || name.length < 4 || name.length > 85) return false;
  if (/^(BIO|NOVINKA|MAX|CENA|ZA|PŘI|KOUPI|BALENÍ|V BALENÍ|A VÍCE|MASA)$/i.test(name)) return false;
  return /[A-ZÁ-Ž]{3}/.test(name);
}

function parsePageProductLine(productLine, pageNumber, sourceUrl) {
  const cleaned = cleanText(productLine, pageNumber);
  ...
  const regex =
    /([A-ZÁ-Ž0-9][A-ZÁ-Ž0-9 %&.,'’\-]{3,}?)(?:\*?)\s+((?:různé druhy|[^<]{0,180}?))?\s*((?:\d+\s*x\s*)?\d+(?:[ ,]\d+)?(?:\/\d+)?\s*(?:g|kg|ml|l|ks|m|svazek|balení)|cena za 1 kg)\s+((?:100\s*g|1\s*kg|1\s*l|100\s*ml|1\s*ks|100\s*ks|1\s*m)\s+\d{1,4}(?:\s?\d{3})*,\d{2}\s*Kč)/g;
  ...
  while ((match = regex.exec(body))) {
    ...
    if (!isProbablyProductName(product)) continue;

    const packageSize = match[3].replace(/\s+/g, " ").trim();
    const unitMatch = match[4].match(/^(.+?)\s+(\d{1,4}(?:\s?\d{3})*,\d{2})\s*Kč$/i);
    const unitPrice = unitMatch ? toNumber(unitMatch[2]) : null;
    const unit = unitMatch ? `Kč/${unitMatch[1].replace(/\s+/g, " ")}` : "Kč/ks";
    const price = leadPrices[priceIndex] ?? null;
    priceIndex += 1;

    offers.push({
      id: makeId(product, price ?? unitPrice ?? 0, pageNumber),
...
async function fetchPage(pageNumber) {
  ...
  const response = await fetch(url, {
    headers: {
      "user-agent": "Mozilla/5.0 (compatible; LetakovyPorovnavacPennyLeafletHtmlImport/0.1; +https://github.com/)",
...
async function main() {
  ...
  for (let pageNumber = 2; pageNumber <= 37; pageNumber++) {
    ...
    page.firstOffers = parsed.offers.slice(0, 20);
  ...
  const meta = {
    ...
    note:
      "První konzervativní import z HTML textu FlippingBook. Položky s confidence=low nemají jistě přiřazenou hlavní cenu.",
  };
