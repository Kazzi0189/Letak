...
function makeOffer(productDef, block) {
  ...
  return {
    id: `penny-page23-v1-${hashId([PAGE_NUMBER, product, price])}`,
    chain: "Penny",
    storeId: "penny-letak",
    ...
    confidence: "targeted-page23-coffee-parser",
    suspect: true,
    suspectReasons: ["candidate z cíleného parseru stránky 23 – ověřit proti letáku"],
...
async function fetchPage() {
  ...
  const response = await fetch(url, {
    redirect: "follow",
    headers: {
      "user-agent": "Mozilla/5.0 (compatible; LetakovyPorovnavacPennyPage23CoffeeProbeV1/0.1; +https://github.com/)",
...
async function main() {
  ...
  const output = {
    meta: {
      source: "Penny page 23 targeted coffee parser v1",
  ...
  const summary = {
    ...
    summary: {
      ...
      recommendedPath:
        offers.length >= 10
          ? "inspect-page23-coffee-candidates-then-import-safe"
          : "adjust-page23-product-starts-or-price-parser",
    },
    offers,
    ...
    pageTextPreview: pageText.slice(0, 6000),
  };

  await writeFile(`${OUTPUT_DIR}/penny-page23-coffee-probe-v1.json`, JSON.stringify(output, null, 2) + "\n", "utf8");
  await writeFile(`${OUTPUT_DIR}/penny-page23-coffee-probe-v1-summary.json`, JSON.stringify(summary, null, 2) + "\n", "utf8");

  console.log("Penny page23 coffee probe v1 finished.");
  console.log(JSON.stringify(summary.summary, null, 2));
  console.log(`Wrote ${OUTPUT_DIR}/penny-page23-coffee-probe-v1-summary.json`);
}
