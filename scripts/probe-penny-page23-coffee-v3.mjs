...
const PAGE23_PRODUCTS = [
  ...
  { start: "KÁVA CASABLANCA OCHUCENÁ", category: "káva", searchTerms: ["káva", "instantní káva", "ochucená káva"] },
...
function makeOffer(productDef, block) {
  ...
  return {
    id: `penny-page23-v3-${hashId([PAGE_NUMBER, product, price])}`,
    chain: "Penny",
    storeId: "penny-letak",
    ...
    confidence: "targeted-page23-coffee-parser-v3",
    suspect: true,
    suspectReasons: ["candidate z cíleného parseru stránky 23 v3 – ověřit proti letáku"],
...
function extractPage23Blocks(pageText) {
  ...
  for (const def of PAGE23_PRODUCTS) {
  ...
  // Odstranění duplicit na stejném indexu; necháme delší start.
  const deduped = [];
...
async function fetchPage() {
  ...
  const response = await fetch(url, {
    redirect: "follow",
    headers: {
      "user-agent": "Mozilla/5.0 (compatible; LetakovyPorovnavacPennyPage23CoffeeProbeV3/0.1; +https://github.com/)",
...
async function main() {
  ...
  const summary = {
    ...
    summary: {
      ...
      hasCasablancaOchucena: offers.some((offer) => normalizeSearch(offer.product).includes("kava casablanca ochucena")),
      ...
      recommendedPath:
        offers.length >= 15 && offers.some((offer) => normalizeSearch(offer.product).includes("kava casablanca ochucena"))
          ? "import-page23-safe-coffee-candidates"
          : "inspect-page23-v3-before-import",
    },
    offers,
  ...
  await writeFile(`${OUTPUT_DIR}/penny-page23-coffee-probe-v3.json`, JSON.stringify(output, null, 2) + "\n", "utf8");
  await writeFile(`${OUTPUT_DIR}/penny-page23-coffee-probe-v3-summary.json`, JSON.stringify(summary, null, 2) + "\n", "utf8");

  console.log("Penny page23 coffee probe v3 finished.");
  console.log(JSON.stringify(summary.summary, null, 2));
  console.log(`Wrote ${OUTPUT_DIR}/penny-page23-coffee-probe-v3-summary.json`);
}
