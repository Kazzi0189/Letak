...
const PENNY = {
  ...
  samplePages: [2, 3, 4, 8, 12, 16, 20, 24, 28, 32, 36],
};
...
async function fetchText(url, accept = "text/html,application/json,text/plain,*/*") {
  const response = await fetch(url, {
    redirect: "follow",
    headers: {
      "user-agent": "Mozilla/5.0 (compatible; LetakovyPorovnavacImageProbe/0.2; +https://github.com/)",
      accept,
...
function findNearbyImageCandidates(html, baseUrl, offers, limit = 30) {
  const imgMatches = Array.from(html.matchAll(/<img\b[^>]*>/gi)).slice(0, 200);
  const imageTags = imgMatches.map((match) => {
    const tag = match[0];
    const src =
  ...
  for (const offer of offers.slice(0, 40)) {
    ...
    const foundIndexes = tokens
      .map((token) => normalizedHtml.indexOf(token))
      .filter((idx) => idx >= 0);

    if (!foundIndexes.length) continue;

    const productIndex = Math.min(...foundIndexes);

    const nearby = imageTags
      .map((img) => ({
        imageUrl: absoluteUrl(img.src, baseUrl),
        distance: Math.abs(img.index - productIndex),
      }))
    ...
    for (const item of nearby) {
      candidates.push({
        ...
        imageClass: classifyImage(item.imageUrl),
      });
    }
  }
...
async function probePenny() {
  ...
  for (const page of PENNY.samplePages) {
    const url = `${PENNY.viewerBaseUrl}${page}/index.html`;
    const response = await fetchText(url);
    const pageOffers = offers.filter((offer) => Number(offer.pageNumber) === page);
    const images = extractImageUrls(response.text, response.finalUrl).slice(0, 60);
    const nearby = findNearbyImageCandidates(response.text, response.finalUrl, pageOffers, 20);
  ...
  const uniqueImages = unique(allImages).slice(0, 60);
  const testedImages = [];
  ...
  return {
    ...
    existingImageFields,
    pages,
    uniqueImages: uniqueImages.map((imageUrl) => ({
      imageUrl,
      imageClass: classifyImage(imageUrl),
    })),
    testedImages,
    nearbyCandidates: nearbyCandidates.slice(0, 80),
  };
}
...
async function probeKaufland() {
  ...
  const offers = data.offers;
  const existingImageFields = extractOfferImageFields(offers);
  const urls = sourceUrlsFromOffers(offers).slice(0, 4);
  ...
  for (const url of urls) {
    const response = await fetchText(url);
    const images = extractImageUrls(response.text, response.finalUrl).slice(0, 60);
    const nearby = findNearbyImageCandidates(response.text, response.finalUrl, offers, 20);
...
async function main() {
  ...
  const summary = {
    ...
    summary: {
      ...
      notes: [
        "V2 je omezený průzkum kvůli paměti: Penny kontroluje vzorek stran a Kaufland jen několik zdrojových URL.",
    ...
    penny: {
      ...
      existingImageFields: penny.existingImageFields.slice(0, 20),
      testedImages: penny.testedImages.slice(0, 60),
      nearbyCandidates: penny.nearbyCandidates.slice(0, 60),
      pages: penny.pages,
    },
    kaufland: {
      offersCount: kaufland.offersCount,
      sourceUrls: kaufland.sourceUrls,
      existingImageFields: kaufland.existingImageFields.slice(0, 20),
      testedImages: kaufland.testedImages.slice(0, 60),
      nearbyCandidates: kaufland.nearbyCandidates.slice(0, 60),
      pages: kaufland.pages,
    },
  };

  await writeFile(`${OUTPUT_DIR}/summary.json`, JSON.stringify(summary, null, 2) + "\n", "utf8");
  await writeFile(
    `${OUTPUT_DIR}/product-image-probe.json`,
    JSON.stringify(
      {
  ...
  console.log("Product image probe v2 finished.");
